...
2. 如果 Hugging Face 不能直连，在“下载环境与维护代理”中填写代理地址，选择 Hugging Face 后执行“测试并保存”。该代理只用于模型目录、依赖和权重下载，不会进入 Router 或 Worker；ModelScope 下载器缺失时由后台任务自动准备固定版本。
3. 从按 ModelScope / Hugging Face 分组的官方 Ornith 原生 GPU 权重中选择目标；页面默认沿用当前部署的 Hub，并优先选择规格相近的 `ornith-ai/Ornith-1.5-35B-A3B-FP8`。revision 可以留空；生成计划时会解析并显示完整不可变 SHA。
4. 保守起步时把最大上下文保持为 `32768`。控制服务使用真实 GPU、显存和模型权重重新规划 TP 与实例数，不照搬 1.0 拓扑。
5. 核对固定 SHA、受影响 Worker、目标 TP、旧权重路径和回退说明。生成计划不会下载权重或停止服务。
6. 安排维护窗口后确认升级。页面会立即收起确认计划并显示任务阶段、进度、消息和日志；新 Worker 健康后，控制器会逐实例显示“第 N/总数”并执行一次有 60 秒上限的真实文本生成，全部通过后才切换公开路由。vLLM 同时接受新的 1.5 内部名和升级前的 1.0 内部名，未及时修改的旧客户端仍可调用。
7. 下载、加载、生成或路由同步失败会自动恢复升级前快照。成功后，任务详情中的“回退到升级前”会重新加载保留的 1.0 权重。
