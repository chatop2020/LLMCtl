...
升级到 LLMCtl 3.6.8 后，进入管理后台“模型部署”，使用首屏的“Qwen3.8 Flash Next 一键部署”：
...
| ModelScope 报 `/root/.modelscope` 只读 | 升级到 3.6.8；控制服务会把 SDK home、cache 和 `--cache-dir` 固定到模型盘私有目录 |
| 下载结束后控制服务持续大量写盘 | 升级到 3.6.8；命令输出持久化已限制为最后 100 行，避免进度条日志写放大 |
| PLE 启动时报缺少 `ngram_embedding.weight_scale` | 升级到 3.6.8；LLMCtl 会从当前基础镜像 ID 构建最小 FP8-PLE resolver 派生层，原镜像和权重保持不变 |
| 在 `starting` 阶段点击安全取消后仍等待 | 升级到 3.6.8；并行 Worker 健康等待现在每 3 秒检查取消标志并立即进入回滚 |
