...
- OmniRoute 模式额外部署 Vue 3 企业门户：支持可配置门户品牌、可选公开基准地址（仅用于生成链接）、邮箱验证/精确企业邮箱后缀/注册开关/SMTP 在线配置、用户与用户组、管理员重置普通用户门户密码并撤销旧登录会话、用户独立且登录后保持不变的 API Key（仅手工轮换时更换）、原生活跃会话上限、每分钟/每日请求上限、预付现金余额、一次性注册赠款、逐模型输入/输出/缓存/思考定价、逐请求扣款账本和审计。余额耗尽后付费模型权限保持关闭，充值后才恢复。
...
命令会先询问是否从 GitHub 获取 `chatop2020/LLMCtl` 的最新 `main`，锁定到明确提交后下载。网络预检会同时检查 GitHub API 与实际归档下载站点；即使预检通过，只要提交信息或 ZIP 的真实传输失败，也会依次尝试已保存的维护代理、询问新的代理并重试。代理只用于本次维护，可选择保存，不会注入推理服务。替换前会备份完整旧控制面；账户门户会短暂停止并验收，模型部署控制服务会重启以加载新能力，失败时自动恢复控制面及本次升级产生的运行数据快照。正常升级不会重启 Router 或 GPU Worker。
...
管理员左侧菜单另有独立的“用量报表”页面，面向全员统计而不是只显示 Top 10。它支持按天、按月、按年和自定义时间范围查询，可按模型、用户关键字和账户状态筛选；汇总区展示全员数、活跃/零用量人数、请求数、输入/输出/总 Token 和现金扣款，并提供时间趋势、模型汇总及包含零用量成员的全员明细。全员明细由数据库分页，不会一次把所有记录载入浏览器。页面可把当前筛选范围内的全部匹配人员导出为真正的 `.xlsx` 工作簿，内含“汇总、用户用量、时间趋势、模型用量”四个工作表；导出不受当前页码限制。
...
- LLMCtl 默认把单次生成硬限制在 `32768` 个输出 Token。该限制由 vLLM Worker 执行；客户端省略 `max_tokens` 或请求更大的数值时也不能生成到 256K 上下文边界。管理员仍可在门户中为公开模型选择更小的最大输出值。
...
- 管理员可在“用户管理”中按需显示、隐藏或复制某个普通用户当前的 API Key。明文不进入管理员快照、门户数据库、日志、审计详情或浏览器持久存储；离开用户页面或退出登录后立即从页面内存清除。
- “邮箱未验证”的注册记录可以由管理员补发验证邮件；新邮件发送成功后旧链接失效。若已通过其他可信方式核实邮箱归属，管理员也可在准确输入完整邮箱后手动通过验证；系统仍会检查注册域名、创建 API Key、发放配置的注册余额、同步模型权限、失效旧验证链接并记录审计，任一步失败都会撤销半成品开户。管理员还可删除没有 API Key、账单、赠额和调用记录的未验证注册占位；正式用户只能停用，不能从该入口删除。
- LLMCtl 会为 OmniRoute 开启详细请求日志，新请求的输入和最终模型输出可在用量详情中受权限控制地查看；历史上未保存的内容无法补录。Router 默认保留 30 天摘要，并为单次请求制品预留 4 MiB；提示词与最终输出分别最多向门户返回 1,000,000 个字符。
- 审计页把内部动作名翻译为管理操作，并允许展开完整详情。极端详情超过 1,000,000 个字符时会明确标记截断，不再静默省略。
...
- 首次启用本版本的详细请求日志时，执行 `sudo llmctl router restart` 让 Router 加载制品大小与保留期环境参数；该命令不重启 GPU Worker。
