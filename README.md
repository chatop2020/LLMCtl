...
- OmniRoute 模式额外部署 Vue 3 企业门户：支持可配置门户品牌、可选公开基准地址（仅用于生成链接）、邮箱验证/精确企业邮箱后缀/注册开关/SMTP 在线配置、用户与用户组、用户独立且登录后保持不变的 API Key（仅手工轮换时更换）、金额余额、额外 token 赠额及周期自动重置、逐模型输入/输出/缓存/思考定价、用量账本和审计。
