...
白名单按 `@` 后完整域名匹配；允许 `example.com` 不会自动允许 `evil-example.com` 或 `dept.example.com`。注册和验证两个阶段都会重新检查，注册页按后端当前配置显示为 `@example.com` 等允许邮箱后缀。门户管理员可在线配置并测试 SMTP、关闭/开启注册、调整白名单、新用户默认赠额和默认 API Key 活跃会话上限，也可按用户覆盖会话上限、禁用用户、分组、增减现金余额、发放通用或指定模型的额外 token。默认赠额可设为 `0`，用于纯现金账户；管理员也可在用户编辑器中一次停用该用户已有的全部有效赠额。
...
赠额支持 `daily`、`weekly`、`monthly` 及指定重置时间；后台会独立执行到期重置。升级后维护线程会自动删除旧版为赠额创建的 OmniRoute 全局 Token Limit，避免“赠额耗尽但仍有现金余额”时 Key 被网关提前拦截。
...
计费不把一笔现金余额粗暴换成一个全局 Token 数，因为不同模型、输入/输出、缓存和思考 Token 的价格可以不同。LLMCtl 以请求实际用量逐类换算标价：有效 Token 赠额优先抵扣有价格的 Token（优先覆盖单价更高的类别，免费类别不浪费赠额），剩余金额才从现金余额扣除。每条账本独立保存“标价、赠额抵扣、余额扣款”和不可变价格快照；以后修改模型价格不会重算历史请求。模型广场会按当前现金余额分别估算该模型的纯输入/纯输出可用 Token，实际扣费仍以请求的真实构成为准。管理员用户列表同时显示现金余额和有效 Token 赠额余额。

用量由门户按请求 ID 从 OmniRoute 持久化调用日志幂等对账，因此余额扣款是异步结算，不是请求前实时预授权；极短时间内可能发生小额超用。原生 RPM/RPD 负责先限制滥用频率，若业务要求严格预付费或严格 TPM，需要在 OmniRoute 数据面实现原子预算预留，而不能在当前直连架构中靠门户事后统计冒充。
