...
The allowlist matches the complete part after `@`; allowing `example.com` does not allow `evil-example.com` or `dept.example.com`. It is checked at both registration and verification, and the registration page renders the current backend values as allowed suffixes such as `@example.com`. Administrators can configure and test SMTP online, enable/disable registration, change the allowlist and welcome grant, disable users, manage groups, adjust cash balances, and issue generic or model-specific token grants. The default grant may be `0` for a cash-only account, and the user editor can retire all of a user's existing active grants in one operation.
...
Grants support `daily`, `weekly`, and `monthly` resets at an explicit time. The maintenance thread also removes legacy OmniRoute global token limits created for old grants, so an exhausted grant can no longer block a key that still has cash balance.
...
A cash balance is not converted into one universal token count because models and token categories can have different prices. LLMCtl prices each request from its actual token classes. Active token grants cover priced tokens first—highest-priced classes first, while free classes do not waste grants—and only the remainder debits cash. Every ledger row stores list price, grant discount, cash debit, and an immutable price snapshot, so later edits never reprice history. The model catalog estimates pure-input and pure-output token capacity from the current cash balance, while actual billing follows the real request mix. The administrator user list shows both cash and active grant-token balances.

Usage is reconciled idempotently from OmniRoute's persisted call logs by request ID, so cash debit is asynchronous accounting rather than request-time authorization; a small overspend window can exist. Native RPM/RPD limits provide the first abuse guard. Strict prepaid or strict TPM would require atomic reservation in the OmniRoute data path and cannot honestly be implemented by portal-side post-processing while `/v1` remains direct.
