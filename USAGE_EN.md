...
- If the same image or PDF request is fast against a Worker but consistently gains about 30 seconds through OmniRoute, it matches OmniRoute's default Combo semaphore timeout. After upgrading LLMCtl, run `sudo llmctl router reconcile` to apply the managed connection/Combo concurrency and one-second failover bounds; GPU Workers do not need to be reinstalled or restarted. Confirm with `sudo llmctl logs router | grep -E 'Semaphore (timeout|queue full)|SEMAPHORE_'`.
