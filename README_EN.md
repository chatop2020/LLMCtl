...
- Administrators can resend verification mail for an unverified registration; the old link is invalidated only after the new message is delivered. After typing the exact email, they may also delete an unverified placeholder that has no API key, billing, grant, or usage history. Provisioned users can only be disabled through this flow.
